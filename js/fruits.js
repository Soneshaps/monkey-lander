class Fruit{
  constructor(){
      ...
      this.fruitImgs = ['./images/cherry.png','./images/lemon.png','./images/pineapple.png'];
  ...
  update(ctx){
    this.image.src = this.fruitImgs[this.fruitNo];
          ...
          for (let index = 0; index < this.position[game.level]?.length; index++) {
              ...
              ctx.drawImage(this.image,0,0,this.imageWidth,this.imageHeight,this.position[game.level][index].x,this.position[game.level][index].y,this.imageWidth,this.imageHeight)
              ctx.closePath()
          }
  ...
  for (
    let index = 0;
    index < this.position[game.level].length;
    index++
  ) {
    if (
